        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (code.length > 2) {
                ...
                if (installedPackagesCodes.contains(code[1] + ".jar")) {
                    ...
                    long localLastModified = ApertiumCaffeine.prefs.getLong("last_modified_" + code[1] + ".jar", -1);
            ...
            public void run() {
                ...
                for (int i = 0; i < packagesToInstall.length; i++)
                    if (packagesToInstall[i]) {
                        try {
                            ...
                            FileOutputStream fos = new FileOutputStream(new File(new File(ApertiumCaffeine.prefs.get("packagesPath", "")), packageToCode.get(tableContent[i][1])));
                            ...
                            fos.close();
                            in.close();
                            ApertiumCaffeine.prefs.putLong("last_modified_" + packageToCode.get(tableContent[i][1]), url.openConnection().getLastModified());
                        } catch (IOException ex) {
                            Logger.getLogger(InstallDialog.class.getName()).log(Level.SEVERE, null, ex);
                ...
                for (int i = 0; i < packagesToInstall.length; i++)
                    if (packagesToUninstall[i]) {
                        ...
                        if (!new File(new File(ApertiumCaffeine.prefs.get("packagesPath", "")), packageToCode.get(tableContent[i][1])).delete()) {
                            JOptionPane.showMessageDialog(InstallDialog.this, "Unable to uninstall " + tableContent[i][1], "Error", JOptionPane.ERROR_MESSAGE);
                        }
                        ApertiumCaffeine.prefs.remove("last_modified_" + packageToCode.get(tableContent[i][1]));
                    }
                dialog.dispose();
            }
        }.start();
